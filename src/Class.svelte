<script lang="ts" context="module">
  ...
  const getTag = (c: ClassInfo) =>
    `${c.modules.map((m) => m.text).join(',')}@${c.time.getTime()}`
  ...
  export const parseClass = (els, startDate, weekday, year): ClassInfo[] => {
    ...
    if (_class.alternativeTimes && _class.alternativeTimes.link) {
      ...
      _class.alternativeTimes.link =
        timetableURL(year) + link.substr(link.indexOf('/Individual'))
    }
...
<div
  class="class"
  class:full
  style="grid-row: {row(info.time)} / span {info.duration /
    interval}; background: {getColour(info.modules)}"
  on:click={clickFull}
>
  <div class="top">
    <p class="modules hide">
      ...
      {:else}
        {#each info.modules as mod, i}
          {#if mod.link}
            <a target="_blank" rel="noopener noreferrer" href={mod.link}
              >{mod.text}</a
