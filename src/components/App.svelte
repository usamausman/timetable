...
<script lang="ts">
	...
	import { add, differenceInDays, format, formatDistance, isSameDay } from 'date-fns';
	import { createEvents } from 'ics';

	import { date, hour, info, line, nextClass, options, timetable } from '$helper/stores';
	import { buildURL, showTime } from '$helper/util';
	import { BUILD_DATE } from '$helper/info';

	import Class, { notify, parseClass, toEvent } from '$comp/Class.svelte';
...
<main class:dark={$options.dark} style="--vh: {vh}px; --segments: {hourSegments};">
	{#if !$info.identifier}
		<div class="info">
			<h1>University of Leeds Timetable</h1>
			<p>
				This app will keep a copy of your timetable so that you can see it even if you are offline!
				You can also see the location of classes, with a link to find them on campus.
			</p>
						...
						{:else}
							<div class="options" on:click|stopPropagation={() => {}}>
								...
								{#if !('Notification' in window)}
									<p style="color: var(--now);">Notifications are not supported.</p>
								{/if}
